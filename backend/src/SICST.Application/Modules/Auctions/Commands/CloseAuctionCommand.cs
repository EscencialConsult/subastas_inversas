...
public record CloseAuctionCommand(Guid CompanyId, Guid AuctionId) : ITenantRequest<AuctionDto?>;
...
public class CloseAuctionCommandHandler : IRequestHandler<CloseAuctionCommand, AuctionDto?>
{
    ...
    public async Task<AuctionDto?> Handle(CloseAuctionCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            throw new BusinessRuleException("La subasta ya esta cerrada.");
        }
        if (auction.Status != AuctionStatus.Open)
        {
            throw new BusinessRuleException("Solo se puede cerrar una subasta abierta.");
        }

...
public class PlaceBidCommandHandler : IRequestHandler<PlaceBidCommand, BidDto>
{
    ...
    public PlaceBidCommandHandler(
        ...
        : this(context, cache, bidLock, null, publicSnapshotCache)
    {
    }
    ...
    public async Task<BidDto> Handle(PlaceBidCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            throw new BusinessRuleException("La subasta no esta habilitada para recibir ofertas.");
        }

        var participant = auction.Participants.FirstOrDefault(p => p.SupplierId == request.SupplierId && p.Active);
        if (participant == null)
        {
            throw new BusinessRuleException("El proveedor no esta habilitado para esta subasta.");
        }
        ...
        {
            throw new BusinessRuleException("Debe esperar a que otro proveedor oferte antes de hacer un nuevo lance.");
        }
        ...
        {
            throw new BusinessRuleException("La oferta debe ser menor al precio actual.");
        }

        if (request.Amount > minimumAmount)
        {
            throw new BusinessRuleException("La oferta no respeta el decremento minimo requerido.");
        }

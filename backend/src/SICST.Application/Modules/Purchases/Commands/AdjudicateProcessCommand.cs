...
public class AdjudicateProcessCommandHandler : IRequestHandler<AdjudicateProcessCommand, PurchaseProcessDto?>
{
    ...
    public AdjudicateProcessCommandHandler(
    ...
    public async Task<PurchaseProcessDto?> Handle(AdjudicateProcessCommand request, CancellationToken cancellationToken)
    {
        ...
        var awardedAtUtc = createdAwards.Max(a => a.AdjudicatedAtUtc);
        _outbox.Add(new ProcessAwarded(
            Guid.NewGuid(),
            request.CompanyId,
            process.Id,
            createdAwards.Select(a => a.Id).ToArray(),
            createdAwards.Sum(a => a.Amount),
            awardedAtUtc,
            DateTime.UtcNow));
